...
- **배포**: Backyard(사내 Kubernetes 샌드박스) `proj-a2qqw2`
...
| BQ 서비스 계정 JSON (`voc-bq-sa.json`) | 로컬 개발·배포 | 데이터팀 (또는 Backyard secret에서 확인) |
...
`02_voc_surge_score.sql`(오늘 기준 view)과 `05_voc_surge_score_at.sql`(TVF)은 정의를 항상 동기화하세요.
